...
def is_validation(user_data, idx):
    errors = list()

    if user_data[idx]['blood_group'] not in blood_types:
        errors.append('blood_group')

    if user_data[idx]['company'] in black_list:
        return 'blocked'

    if '@' not in user_data[idx]['mail']:
        errors.append('mail')

    if not (2 <= len(user_data[idx]['name']) <= 30):
        errors.append('name')

    if not user_data[idx]['website']:
...
def create_user(user_data):
    ...
    for idx in range(len(user_data)):
        result = is_validation(user_data, idx)

        if result == True:
            user_list.append(user_data[idx])
        elif result == 'blocked':
            false_user += 1
        else:
            false_user += 1
            for idx2 in range(len(result[1])):
                user_data[idx][result[1][idx2]] = None
            user_list.append(user_data[idx])

    print(f'잘못된 데이터로 구성된 유저의 수는 {false_user} 입니다.')
    return user_list
